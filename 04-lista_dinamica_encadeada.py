...
#O Código acima foi produzido pelo professor Tiago Ferreira
#e fornecido pelo professor Cícero Garrozzi para realização deste exercício
...
while posicao < len(entrada):
    if entrada[posicao] == 1:
        posicao+=1
        lista_encadeada.insertAtBegin(entrada[posicao])
    elif entrada[posicao] == 2:
        posicao+=1
        lista_encadeada.insertAtEnd(entrada[posicao])
    elif entrada[posicao] == 3:
        lista_encadeada.removeFromBegin()
    elif entrada[posicao] == 4:
        lista_encadeada.removeFromEnd()
    posicao+=1

#Saída de dados
#deverá imprimir o resultado da função getAsText() do seu objeto List
print(lista_encadeada.getAsText())